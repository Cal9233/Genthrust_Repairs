...
const logger = createLogger('ExcelSession');
 ...
 *   await callGraphAPI(endpoint, 'GET', undefined, sessionId);
 * });
 * ```
 */
export class ExcelSessionManager {
  ...
  constructor(
    ...
    logger.debug('ExcelSessionManager initialized', {
      driveId,
      fileId,
  ...
  async executeWithSession<T>(
    ...
    return await this.withSession(async (sessionId) => {
      return await this.callGraphAPI(endpoint, method, body, sessionId);
    });
  }
